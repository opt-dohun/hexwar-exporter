...
* **Loki + Promtail 실시간 로그 수집:** 클러스터 내부 Pod의 로그를 Promtail을 통해 수집하고, Loki Agent에 전달하도록 구성하였습니다. Loki는 로그 데이터를 저장하고 시간 단위로 chunk를 생성함과 동시에 인덱스 처리 비용이 저렴하여 실시간으로 로그를 수집하고 검색하는 데 유리합니다. 이를 통해 Quickwit의 인덱스 배치 지연시간 단점을 완화 할 수 있습니다.
* **QuickWit 장기 보관 및 분석 환경:** 자체적인 검색 UI를 통해 시계열 분석과 로그 검색을 모두 지원합니다. 역색인을 지원하되 검색 노드에 인덱스를 상주시키는 것이 아닌 메타데이터를 기반으로 인덱스 파일을 로드하는 방식을 사용하여, 컴퓨팅 자원을 효율적으로 사용함과 동시에 Loki 대비 검색 성능이 빠르며 또한 파일시스템 디스크 보관이 아닌 객체 저장소를 통해 장기적인 데이터 보관이 가능합니다.
* **데이터 파이프라인 구축:** vector를 통해 수집된 파일에서 라벨을 분리하고, kafka를 사용해 로그를 적재하였습니다. 이후 Quickwit에서 인덱스 배치 작업을 위해 일정 단위만큼 소비하도록 구현하여, 인덱스 노드의 소비 속도에 맞추어 로그를 소비할 수 있는 파이프라인을 구축하였습니다.

#### 결과 및 검증 데이터

1. **Quickwit 로그 데이터 압축 성능 검증**
   * **검증 내용:** Quickwit의 객체 저장소 기반 인덱싱 및 데이터 압축 효율성을 실측 데이터 기반으로 검증했습니다.
   * **압축 성능 실측 데이터:**
     | 구분 | 1,000건 데이터 (환산) | 전체 누적 데이터 (1,020만 건 실측) |
     | :--- | :---: | :---: |
   ...
   * **결과:** Quickwit이 키워드 검색을 위한 역색인을 생성 및 저장을 진행하였으나, 인덱스 파일 기반 압축 및 청크 단위 저장을 통해 원본 대비 약 79.4%의 용량을 절감한 것을 확인하였습니다. 

### 3. 메트릭 수집을 위한 사이드카 패턴 구축 
